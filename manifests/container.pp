define lxc::container(
  ...
  $lxc_cgroup_cpuset_cpus                 = $lxc::defaults::lxc_cgroup_cpuset_cpus,
  $lxc_cgroup_cpu_shares                  = $lxc::defaults::lxc_cgroup_cpu_shares,
) {
  if $unprivileged {
  ...
  case $ensure {
    'present', 'running': {
      if $ensure == "running" or $start_on_creation {
        if $unprivileged {
          notify {"starting ${utsname} with user ${unprivileged_user} with home in ${unprivileged_home}": }
          exec { "lxc-start ${utsname}":
            command       => "/bin/su - ${unprivileged_user} -c \"${lxc_start}\"",
            unless        => "${lxc_info} | grep \"State.*RUNNING\"",
            logoutput     => 'on_failure',
            #environment   => ["HOME=${unprivileged_home}", "USER=${unprivileged_user}"],
            provider      => 'shell',
            #user          => $unprivileged_user,
            require       => [Exec["lxc-create ${utsname}"],
                              File[$config_file],
